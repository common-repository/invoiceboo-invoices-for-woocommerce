...
//
// Description :
//   Static methods used by the InvoiceBoo_TCPDF class.
//
//============================================================+

/**
 * @file
 * This is a PHP class that contains static methods for the InvoiceBoo_TCPDF class.<br>
...
class InvoiceBoo_TCPDF_STATIC {
	...
	/**
	 * Current InvoiceBoo_TCPDF version.
	 * @private static
	 */
	...
	/**
	 * Return the current InvoiceBoo_TCPDF version.
	 * @return string InvoiceBoo_TCPDF version string
	 * @since 5.9.012 (2010-11-10)
	 * @public static
	 */
	public static function getInvoiceBoo_TCPDFVersion() {
		return self::$tcpdf_version;
	}

	/**
	 * Return the current InvoiceBoo_TCPDF producer.
	 * @return string InvoiceBoo_TCPDF producer string
	 * @since 6.0.000 (2013-03-16)
	 * @public static
	 */
	public static function getInvoiceBoo_TCPDFProducer() {
		return "\x54\x43\x50\x44\x46\x20".self::getInvoiceBoo_TCPDFVersion()."\x20\x28\x68\x74\x74\x70\x3a\x2f\x2f\x77\x77\x77\x2e\x74\x63\x70\x64\x66\x2e\x6f\x72\x67\x29";
	}
	 ...
	 * @param string $file_id InvoiceBoo_TCPDF file_id.
	 * @return string filename.
	 * @since 4.5.000 (2008-12-31)
	 * @public static
	 */
	public static function getObjFilename($type='tmp', $file_id='') {
		return tempnam(K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(InvoiceBoo_TCPDF_STATIC::getRandomSeed()).'_');
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		if (isset($prop['fillColor'])) {
			if (is_array($prop['fillColor'])) {
				...
				$opt['mk']['bg'] = InvoiceBoo_TCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
			}
		}
		...
		if (isset($prop['strokeColor'])) {
			if (is_array($prop['strokeColor'])) {
				...
				$opt['mk']['bc'] = InvoiceBoo_TCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
			}
		}
	...
	public static function fixHTMLCode($html, $default_css, $tagvs, $tidy_options, &$tagvspaces) {
		...
		if (InvoiceBoo_TCPDF_STATIC::empty_string($tidy_options)) {
			$tidy_options = array (
				'clean' => 1,
				'drop-empty-paras' => 0,
		...
		if (!InvoiceBoo_TCPDF_STATIC::empty_string($tagvs)) {
	...
	public static function getHyphenPatternsFromTEX($file) {
		...
		foreach($patterns_array as $val) {
			if (!InvoiceBoo_TCPDF_STATIC::empty_string($val)) {
				$val = trim($val);
				$val = str_replace('\'', '\\\'', $val);
...
} // END OF InvoiceBoo_TCPDF_STATIC CLASS
