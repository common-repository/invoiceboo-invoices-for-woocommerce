...
// -----------------------------------------------------------------------------
	...
	protected function decodeStream($sdic, $stream) {
		...
		foreach ($filters as $filter) {
			if (in_array($filter, InvoiceBoo_TCPDF_FILTERS::getAvailableFilters())) {
				try {
					$stream = InvoiceBoo_TCPDF_FILTERS::decodeFilter($filter, $stream);
				} catch (Exception $e) {
					$emsg = $e->getMessage();
	...
	public function Error($msg) {
		if ($this->cfg['die_for_errors']) {
			die('<strong>InvoiceBoo_TCPDF_PARSER ERROR: </strong>'.$msg);
		} else {
			throw new Exception('InvoiceBoo_TCPDF_PARSER ERROR: '.$msg);
		}
	}

} // END OF InvoiceBoo_TCPDF_PARSER CLASS
